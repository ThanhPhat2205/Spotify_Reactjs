...
const Bodycontent: React.FC=()=>{
    return(
        ...
        <div className="flex gap-4 overflow-x-auto sm:grid sm:grid-cols-6 md:grid-cols-6 lg:grid-cols-6 sm:overflow-x-hidden">
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={index}
              className=" w-40 bg-[#2A2A2A] rounded-md p-3 hover:bg-[#333333] transition"
            >
              <img
                src={anh}
                alt={`Album ${index + 1}`}

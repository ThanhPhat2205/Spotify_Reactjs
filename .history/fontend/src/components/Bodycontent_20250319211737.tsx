    ...
    return(
        ...
        <div className="flex gap-4 overflow-x-auto  md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 md:overflow-x-hidden">
          {["Sơn Tùng M-TP", "Soobin", "HIEUTHUHAI", "Jack", "hieu thu 3", "Rose"].map((artist, index) => (
            <div key={index} className="justify-center items-center flex-shrink-0 w-40 bg-[#2A2A2A] rounded-md p-3 hover:bg-[#333333] transition">
              <img
                src={anh2}
                alt={`Artist ${index + 1}`}
                className="w-20 justify-center items-center h-20 rounded-full object-cover"
              />
